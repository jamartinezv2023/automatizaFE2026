...
public class ConsultarRastreoConGuiaInexistente implements Task {

    @Override
    public <T extends Actor> void performAs(T actor) {
        actor.attemptsTo(
                WaitUntil.the(RastreoPage.CAMPO_GUIA, isVisible()).forNoMoreThan(20).seconds(),
                Enter.theValue("GUIA-INEXISTENTE-999999").into(RastreoPage.CAMPO_GUIA),
                WaitUntil.the(RastreoPage.BOTON_RASTREAR, isEnabled()).forNoMoreThan(20).seconds(),
                Click.on(RastreoPage.BOTON_RASTREAR)
        );
    }

    public static ConsultarRastreoConGuiaInexistente enFormulario() {
        return instrumented(ConsultarRastreoConGuiaInexistente.class);
    }
}
